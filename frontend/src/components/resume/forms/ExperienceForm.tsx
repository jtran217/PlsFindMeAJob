...
export function ExperienceForm({ experience, onSave, onCancel }: ExperienceFormProps) {
  ...
  const [draft, setDraft] = useState<Experience>(() =>
    experience ? { ...experience, bullet_points: experience.bullet_points.map((b) => ({ ...b })) } : emptyExperience()
  )
  ...
  const updateField = (field: keyof Omit<Experience, 'id' | 'bullet_points' | 'overall_keywords' | 'relevance_score'>, value: string) => {
  ...
  const handleSave = () => {
    // Strip out blank bullets before saving
    const cleaned = { ...draft, bullet_points: draft.bullet_points.filter((b) => b.text.trim()) }
    onSave(cleaned)
  }

  const topFields: { key: keyof Omit<Experience, 'id' | 'bullet_points' | 'overall_keywords' | 'relevance_score'>; label: string; placeholder: string }[] = [
    { key: 'title',    label: 'Job Title',  placeholder: 'Software Engineer' },
    { key: 'company',  label: 'Company',    placeholder: 'Acme Corp' },
    { key: 'location', label: 'Location',   placeholder: 'San Francisco, CA' },
    { key: 'duration', label: 'Duration',   placeholder: 'June 2022 -- Present' },
  ]
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl rounded-2xl border border-slate-800/80 bg-[#0b1228] shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-slate-800/70 shrink-0">
          ...
          <h2 className="mt-0.5 text-lg font-semibold text-white">Work Experience</h2>
        </div>
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="text-xs text-slate-600">{draft.bullet_points.length} bullet{draft.bullet_points.length !== 1 ? 's' : ''}</span>
            </div>
                  ...
                  {bulletEditingId === bullet.id ? (
                    <div className="p-3">
                      <textarea
                        autoFocus
                        value={bullet.text}
                        placeholder="Describe an achievement or responsibility using action verbs..."
